...
function isInboxOptimisticMessage(id: string): boolean {
  return id.startsWith("temp-") || isInboxMessagePreview(id);
}
...
export const ChatMessage = memo(function ChatMessage({
  ...
  const [resolvedText, setResolvedText] = useState<string | null | undefined>(
    message.text,
  );
  ...
  const needsResolve =
    Boolean(message.id) &&
    !isInboxOptimisticMessage(message.id) &&
    ...
    resolveInboxMessageMedia(message.id)
      .then((row) => {
        ...
        if (row.attachmentUrls?.length)
          setResolvedUrls(dedupeMediaUrls(row.attachmentUrls));
        else if (row.attachmentUrl)
          setResolvedUrls(dedupeMediaUrls([row.attachmentUrl]));
  ...
  const imageUrls =
    media.messageType === "image"
      ? resolvedUrls.filter(
          (u) =>
            (u.startsWith("http") || u.startsWith("blob:")) &&
            !failedUrls.has(u),
        )
      : [];
  const videoUrl =
    media.messageType === "video"
      ? (resolvedUrls[0] ?? media.attachmentUrl)
      : null;
              ...
              {showVi && (
                <div
                  className={cn(
                    ...
                    isOwn
                      ? "border-white/25 text-blue-50/90"
                ...
                >
                  <span
                    className={cn(
                      "font-medium",
                      isOwn ? "text-blue-50" : "text-slate-600",
                    )}
                  >
